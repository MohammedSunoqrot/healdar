...
| "Healdar has used today's free AI allowance" | Both models' daily allowances are used up. The free plan gives `gpt-oss-120b` 200,000 tokens a day (about 40 questions); after that answers come from the backups (`qwen3.8-27b`, then `gpt-oss-20b`) until their allowances run out too | Wait for the time the message shows, or upgrade the Groq plan |
